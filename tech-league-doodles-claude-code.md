# Task: add hand drawn notebook doodles to the Tech League homepage

## Goal

The ColorStack Tech League homepage has a lot of empty space on the left and right sides of every section. Fill that space with small hand drawn, notebook style objects (think doodles in the margin of a notebook).

**Do not change anything that already exists.** No edits to existing components, text, layout, colors, fonts, or CSS. This task only adds two new files and three lines of code to mount them.

## Rules

1. Do not edit any existing JSX, CSS, or content. Only add the files below and the mount code in step 3.
2. Copy both files exactly as written. Do not restyle, rename classes, or "improve" them.
...
4. If a selector in `LAYOUT` does not match the site's markup, report it and fix only that selector. Do not change the markup to fit the script.

## What gets added

Decorative SVG doodles placed in the side gutters of each section, plus one dashed "top of the board?" trophy card that fills the empty grid spot next to the Capstone Hackathon card.

- **Hero:** to do notebook, pencil, "only takes 5 min!" arrow, laptop, JAVA mug, crumpled paper, paper plane, sparkles, code scribbles
- **Challenges:** rubber duck ("debug buddy"), ctrl + Z keycaps ("one bad round? undo."), O(n log n) sticky note, lightbulb, trophy card in the empty grid cell
...
All doodles are `aria-hidden`, don't block clicks on real content, and turn off their motion when the user has reduced motion enabled.

## Steps

### 1. Create `src/doodles/doodles.js`
...
### 2. Create `src/doodles/doodles.css`
...
### 3. Mount it in the component that renders the homepage (usually `src/App.jsx`)
...
If `useEffect` is already imported, don't import it twice. `mountDoodles()` returns a cleanup function, so it is safe with React StrictMode.

## Selectors the script depends on

These already exist on the site. Confirm each one matches before finishing:

| Selector | Section |
|---|---|
| `section.hero` | Hero |
| `#challenges` | The five challenges |
| `.challenges__grid` (inside `#challenges`) | Grid that gets the trophy card |
| `#timeline` | How it runs |
| `#partners` | Built together |
| `section.section:last-of-type` | Ready to apply |

Each of these sections must have `position: relative` (they already do). If one doesn't, report it rather than changing the section.

## Check your work

Run the dev server and confirm:

- [ ] Nothing in the original page moved, changed color, or changed size
- [ ] At 1440px wide, doodles appear beside every section and never cover text or cards
- [ ] At 1920px wide, the extra doodles near the far edges also appear
- [ ] Below 1360px wide, side doodles are hidden
- [ ] The trophy card fills the empty third spot in the second row of the challenges grid
- [ ] The trophy card is hidden below 700px wide
- [ ] All buttons and links still work
- [ ] No console errors, and no horizontal scrollbar at any width

## Tweaking positions later

Each item in `LAYOUT` has:

- `e`: distance in px from the center of the page to the doodle's inner edge
- `t`: distance in px from the top of its section
- `w`: width in px
- `r`: rotation in degrees
- `tier`: `near` shows at 1360px and wider, `far` only at 1700px and wider
- `label` / `labelTop`: optional handwritten caption below or above the doodle
- `a`: optional motion (`bob`, `drift`, `twinkle`, `ring`, `glow`)
